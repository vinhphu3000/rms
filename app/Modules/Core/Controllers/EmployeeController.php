...
use App\Modules\Core\Models\EmployeeExp;
use App\Modules\Core\Models\EmployeeExpMatrix;
use App\Modules\Core\Models\EmployeeProposal;
use App\Modules\Core\Models\Project;
use App\Modules\Core\Models\ProjectBooking;
use App\Modules\Core\Models\Proposal;
...
use App\Modules\Core\Models\Employee as EmployeeModel;
use Symfony\Component\HttpFoundation\File\Exception\FileException;

class EmployeeController extends ModulesController {
    ...
    public function profile($id)
    {
        ...
        return $this->viewModule('employee.profile',['employee' => $employee, 'result' => Project::all(), 'projects_booking' => $projects_booking, 'skill' => $skill, 'activity' => $activity]);
    }
    ...
    public function doSearch(\Illuminate\Http\Request $request)
    {
        ...
        return $this->viewModule('employee.search',['employees' => $employees, 'result' => Project::all()]);
    }
    /**
     * listing action
     */
    public function listing(\Illuminate\Http\Request $request)
    {
        ...
        $employee_exp = \App\Modules\Core\Models\EmployeeExp::all();
        return $this->viewModule('employee.list', ['result' => $result, 'db_column' => $column, 'search_param' => $search_param, 'employee_exp' => $employee_exp]);
    }
            ...
            foreach($results as $row)
            {
                ...
                foreach (EmployeeModel::$csv_map_column as $item_map) {
                    ...
                    if ($item_map->db_column == 'position_id' && !is_numeric($row->{$item_map->csv_column})) {
                        $employeePosition = \App\Modules\Core\Models\EmployeePosition::firstOrCreate(array('name' => $row->{$item_map->csv_column}));
                        $employee->position_id = $employeePosition->id;
                        continue;
                    }

                    if ($item_map->db_column == 'office_id' && !is_numeric($row->{$item_map->csv_column})) {
                        $employeePosition = \App\Modules\Core\Models\Office::firstOrCreate(array('name' => $row->{$item_map->csv_column}));
                        $employee->office_id = $employeePosition->id;
                        continue;
                    }

                    if ($item_map->db_column == 'role_id' && !is_numeric($row->{$item_map->csv_column})) {
                        $employeePosition = \App\Modules\Core\Models\EmployeeRole::firstOrCreate(array('name' => $row->{$item_map->csv_column}));
    ...
    {
        $employee_exp = \App\Modules\Core\Models\EmployeeExp::orderBy('type')->get();
        ...
        if (count($request['employee_ids'])) {
            ...
            $experience = \App\Modules\Core\Models\EmployeeExpMatrix::where('employee_id', $employee_selected->id)->get();
            $next_employee_ids = [];
            ...
            return $this->viewModule('employee.experience', ['employee_exp' => $employee_exp, 'employee' => $employee_selected, 'cv_file' => $cv_file, 'experience' => $experience, 'next_employee_ids' => $next_employee_ids, 'prev_employee_ids' => $prev_employee_ids]);
        }
    ...
    {
        $column = Employee::getTableColumns();
        return $this->viewModule('employee.import', ['db_column' => $column]);
    }
    ...
    public function doExperienceSave(\Illuminate\Http\Request $request)
    {
        ...
        foreach ($experience_data as $item) {
            if (!empty($item->id)) {
                $employeeExpMatrix = \App\Modules\Core\Models\EmployeeExpMatrix::find((int)$item->id);
                if (empty($employeeExpMatrix->id)) {
                    $employeeExpMatrix = new \App\Modules\Core\Models\EmployeeExpMatrix();
                } else {
            ...
            $id_not_deleted[] = $employeeExpMatrix->id;
            $employee_exp = \App\Modules\Core\Models\EmployeeExp::find($item->exp_id);
         ...
         */
        \App\Modules\Core\Models\EmployeeExpMatrix::where('employee_id', $employee_id)->whereNotIn('id', $id_not_deleted)->delete();
